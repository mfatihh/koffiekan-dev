...
class CartController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $queriedProducts = Product::with('unit')->get();
        //dd($queriedProducts);
    ...
    public function show(Request $request, $draftKey)
    {
        ...
        $queriedProducts = [];
        if ($query) {
            $queriedProducts = Product::where('unit_id', $query)->get();
        }else{
            $queriedProducts = Product::with('unit')->get();
        }
        return view('cart.index', compact('draft', 'queriedProducts'));
    ...
    public function proccess(Request $request, $draftKey)
    {
        ...
        $draft = $this->cart->updateDraftAttributes($draftKey, $request->only('type', 'payment','tanggal'));
    ...
    public function store(Request $request, $draftKey)
    {
        ...
        return redirect()->route('transactions.show', $transaction->invoice_no)->with('message', 'Transaksi berhasil');
    }
}
